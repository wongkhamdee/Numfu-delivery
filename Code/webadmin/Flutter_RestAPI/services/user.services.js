...
async function login({ email, password }, callback) {
  let selectQuery = 'SELECT COUNT(*) as "total", ?? FROM ?? WHERE ?? = ? LIMIT 1';
    let query = mysql.format(selectQuery, ["password", "admin", "email", email]);
...
async function register(params, callback) {
  ...
  let selectQuery = 'SELECT Count(*) as total FROM ?? WHERE ?? = ? LIMIT 1';
  let query = mysql.format(selectQuery, ["admin", "Email", params.email]);
  ...
  db.query(query, (err, data) => {
    ...
    if (data[0].total > 0) {
        return callback({message : "Email already exist!"
    });
    }
    else{
        db.query(
          `INSERT INTO admin(username, email, password) VALUES  (?, ?, ?)`,
            ...
            (error, results, fields) => {
                ...
                return callback(null, "Admin Register Successfully")
            }
        )
    }
});
